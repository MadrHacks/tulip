...
func TestSnapshotRestorePreservesMatching(t *testing.T) {
	...
	idA, _ := cs.Assign(a, 1000, false, 0)
	idB, _ := cs.Assign(b, 1000, false, 0)
	for i := 0; i < 5; i++ {
		cs.Assign(a, 1000, false, 0)
	}
	...
	if id, isNew := restored.Assign(a, 1000, false, 0); isNew || id != idA {
		t.Errorf("restored A assign = (%d,%v), want (%d,false)", id, isNew, idA)
	}
	if id, isNew := restored.Assign(b, 1000, false, 0); isNew || id != idB {
	...
	if id, isNew := restored.Assign(c, 1000, false, 0); !isNew || id <= idB {
