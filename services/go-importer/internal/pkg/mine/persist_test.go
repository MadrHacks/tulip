...
func TestSnapshotRestorePreservesMatching(t *testing.T) {
	...
	idA, _ := cs.Assign(a, 1000, false)
	idB, _ := cs.Assign(b, 1000, false)
	for i := 0; i < 5; i++ {
		cs.Assign(a, 1000, false)
	}
	...
	if id, isNew := restored.Assign(a, 1000, false); isNew || id != idA {
		t.Errorf("restored A assign = (%d,%v), want (%d,false)", id, isNew, idA)
	}
	if id, isNew := restored.Assign(b, 1000, false); isNew || id != idB {
	...
	if id, isNew := restored.Assign(c, 1000, false); !isNew || id <= idB {
